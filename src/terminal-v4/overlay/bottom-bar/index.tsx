...
interface BottomBarProps {
  ...
  mapDisabled?: boolean;
  /** Security Mode is on: the shield reads as pressed, and every OTHER circle
   *  greys out. The mode owns the camera. It is parked at the overview with a
   *  remembered way back, and Home, Dollhouse, First Person and the map each
   *  move the operator somewhere that return no longer describes. Greyed rather
   *  than removed, per the note on `mapDisabled`: the shield is the way out,
   *  and it stays exactly where the thumb left it. */
  securityActive?: boolean;
  /** Toggle Security Mode. Omitted when the site has no executive overview to
   *  show, and the circle then simply isn't drawn, the same rule First Person
   *  follows. */
  onSecurity?: () => void;
}
 ...
 * instructions, stand somewhere walkable (First Person), enter Security Mode,
 * or return home.
 *
