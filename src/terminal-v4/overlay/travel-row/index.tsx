...
interface TravelRowProps {
  ...
  onWalk?: () => void;
  /**
   * Listed, but not a destination yet.
   *
   * Dimmed and unpressable rather than hidden: a capability that exists in the
   * data but has nowhere finished to go is still part of the set, and removing
   * the row would make the layer look shorter than it is. `disabled` on the
   * button carries it to the keyboard and to a screen reader too, which
  ...
  return (
    ...
    >
      <button
        ...
        // `button { cursor: default }`, so every control in this tree has to ask
        // for the pointer. This row is the panel's PRIMARY target and was the
        // one place that did not — the hairline walk button beside it did, which
        // made one half of a single row feel clickable and the other half not.
        className={cn(
