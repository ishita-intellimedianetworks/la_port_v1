...
export interface PlayerControllerHandle {
  ...
  startIdleDrift: () => void;
  /** Cut the idle drift short. The drift already ends itself after one full
   *  turn and any drag cancels it; this is for the cases where the UI, not the
   *  pointer, takes the camera over. Security Mode parks it at a composed
   *  overview, and a view still rotating under a layer being read is wrong. */
  stopIdleDrift: () => void;
  /**
