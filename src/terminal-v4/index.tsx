...
/**
 * TerminalExperienceV4 — the Everport digital twin, rendered at /v4.
 *
 * A full fork of `src/terminal-v3`, byte-identical when it was made. Edits here
 * cannot reach `/`, `/v2` or `/v3`. Do not re-export one tree from the other —
 * that restores the coupling this fork exists to remove.
 *
 ...
 *
 * It runs the `v4` model: `config/sites/v4.json` — its own document, its own
 * cameras, its own bake. Nothing it holds is read by `/`, `/v2` or `/v3`.
 */
...
interface TerminalExperienceProps {
  ...
  nodeId?: string;
  onReady?: () => void;
  /** WHICH MODEL to run — the site file read and the bake streamed. `v4` is
   *  this route's own document; the prop exists so the tree cannot silently
   *  read someone else's. */
  site?: SiteId;
}

export default function TerminalExperienceV4({
  nodeId = SITE_NODE_ID,
  onReady,
  site = "v4",
